...
def process_test_image_v2(image_path, config, sens_umbral=0.05):
    ...
    if config.get('crop_rect') is None:
        # Si viene del cropper manual, NO buscamos contornos, usamos la imagen entera
        img_final = img.copy()
    else:
        # --- ESCÁNER ROBUSTO (Solo para fotos automáticas/completas) ---
        ...
        if cnts:
            ...
            img_final = img.copy() # Respaldo por si no encuentra contornos
